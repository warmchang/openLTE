...
int T_PollRetransmit::Pack(std::vector<uint8_t> &bits)
{
    if(!present)
    {
        printf("T_PollRetransmit::Pack() presence failure\n");
        return -1;
    }
    uint32_t N_bits = 6;
...
int T_PollRetransmit::Unpack(std::vector<uint8_t> bits)
{
    uint32_t idx = 0;
    return Unpack(bits, idx);
}

int T_PollRetransmit::Unpack(std::vector<uint8_t> bits, uint32_t &idx)
{
    uint32_t packed_val = 0;
    uint32_t N_bits = 6;
    if((idx + N_bits) > bits.size())
    {
        printf("T_PollRetransmit::Unpack() index out of bounds for value\n");
        return -1;
    }
    for(uint32_t i=0; i<N_bits; i++)
    ...
    if(packed_val > 63)
    {
        printf("T_PollRetransmit::Unpack() max failure\n");
        return -1;
    }
...
T_PollRetransmit::Enum T_PollRetransmit::Value() const
{
    if(present)
        return internal_value;
    return (T_PollRetransmit::Enum)0;
}

int T_PollRetransmit::SetValue(Enum value)
{
...
int T_PollRetransmit::SetValue(std::string value)
{
    if("ms5" == value)
    {
...
std::string T_PollRetransmit::ValueToString(Enum value) const
{
    if(k_ms5 == value)
        return "ms5";
    if(k_ms10 == value)
...
uint64_t T_PollRetransmit::NumberOfValues() const
{
    return 64;
}

std::string T_PollRetransmit::ToString(uint32_t indent) const
{
    std::string out;
    for(uint32_t i=0; i<indent; i++)
        out += "    ";
    out += "T_PollRetransmit = " + ValueToString(internal_value) + "\n";
    return out;
}

std::string T_PollRetransmit::ToStringNoNewLines() const
{
    std::string out = "T_PollRetransmit=" + ValueToString(internal_value) + ",";
    return out;
}

int T_PollRetransmit::Clear()
{
    present = false;
    return 0;
}

bool T_PollRetransmit::IsPresent() const
{
    return present;
}
