...
int DedicatedInfoNAS::Pack(std::vector<uint8_t> &bits)
{
    if(!present)
    {
        printf("DedicatedInfoNAS::Pack() presence failure\n");
        return -1;
    }
...
int DedicatedInfoNAS::Unpack(std::vector<uint8_t> bits)
{
    uint32_t idx = 0;
    return Unpack(bits, idx);
}

int DedicatedInfoNAS::Unpack(std::vector<uint8_t> bits, uint32_t &idx)
{
    uint32_t local_size_length = 0;
    uint32_t size = 0;
    if(0 == bits[idx++])
    ...
    {
        printf("DedicatedInfoNAS::Unpack() index out of bounds\n");
        return -1;
    }
...
std::vector<uint8_t> DedicatedInfoNAS::Value() const
{
    if(present)
...
int DedicatedInfoNAS::SetValue(std::vector<uint8_t> value)
{
...
std::string DedicatedInfoNAS::ToString(uint32_t indent) const
{
    std::string out;
...
std::string DedicatedInfoNAS::ToStringNoNewLines() const
{
...
int DedicatedInfoNAS::Clear()
{
    present = false;
    return 0;
}

bool DedicatedInfoNAS::IsPresent() const
{
    return present;
}
